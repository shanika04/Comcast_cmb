...
/**
 * Wrapper class for all CQS API calls as a layer to call CQS APIs within the same JVM. Eventually
 * all calls coming in through Jetty should go through this layer.
 *
 * @author boris
 */
public class CQSAPI {
  ...
  private static void emitLogLine(
      String userId, String action, String queueUrl, List<String> receiptHandles, long rt) {
    ...
    // logLine.append("
    // cass_ms=").append(CMBControllerServlet.valueAccumulator.getCounter(AccumulatorName.CassandraTime));
    // logLine.append("
    // cass_num_rd=").append(CMBControllerServlet.valueAccumulator.getCounter(AccumulatorName.CassandraRead));
    // logLine.append("
    // cass_num_wr=").append(CMBControllerServlet.valueAccumulator.getCounter(AccumulatorName.CassandraWrite));
    // ogLine.append("
    // redis_ms=").append(CMBControllerServlet.valueAccumulator.getCounter(AccumulatorName.RedisTime));
    // logLine.append("
  ...
  public static String sendMessage(
    ...
    emitLogLine(
        ...
        "SendMessage",
        relativeQueueUrl,
        new ArrayList<String>(Arrays.asList(receiptHandle)),
        ts2 - ts1);

    return receiptHandle;
  }

  public static List<CQSMessage> receiveMessages(
      String userId,
      String relativeQueueUrl,
  ...
  public static void deleteMessage(String userId, String relativeQueueUrl, String receiptHandle)
      throws Exception {
    long ts1 = System.currentTimeMillis();
  ...
  public static CQSQueue createQueue(
      String userId,
      String queueName,
      Integer visibilityTimeout,
    ...
    if (queueName.length() > CMBProperties.getInstance().getCQSMaxNameLength()) {
      throw new CMBException(
          ...
          "QueueName "
              ...
              + CMBProperties.getInstance().getCQSMaxNameLength());
    }
    ...
    if (!m.matches()) {
      throw new CMBException(
          ...
          "QueueName "
              + queueName
              + " is invalid. Only alphanumeric and hyphen and underscore allowed.");
    }
    ...
    if (messageRetentionPeriod != null) {
      if (messageRetentionPeriod < 0
        ...
        throw new CMBException(
            CMBErrorCodes.InvalidParameterValue,
            CQSConstants.MESSAGE_RETENTION_PERIOD
                + " should be between 0 and "
                + CMBProperties.getInstance().getCQSMaxMessageRetentionPeriod());
      }
    ...
    if (receiveMessageWaitTimeSeconds != null) {
      if (receiveMessageWaitTimeSeconds < 0
        ...
        throw new CMBException(
            CMBErrorCodes.InvalidParameterValue,
            CQSConstants.RECEIVE_MESSAGE_WAIT_TIME_SECONDS
                + " should be between 0 and "
                + CMBProperties.getInstance().getCQSMaxMessageDelaySeconds());
      }
    ...
    for (int shard = 0; shard < numberOfShards; shard++) {
      PersistenceFactory.getCQSMessagePersistence()
          .checkCacheConsistency(queue.getRelativeUrl(), shard, false);
    }
  ...
  public static void changeMessageVisibility(
      String userId, String relativeQueueUrl, String receiptHandle, Integer visibilityTimeout)
      throws Exception {
    ...
    PersistenceFactory.getCQSMessagePersistence()
        .changeMessageVisibility(queue, receiptHandle, visibilityTimeout);
