...
public interface ICQSMessagePersistence {
  ...
  /**
   * Create a batch of messages on a specific shard and a random partition. Note: THe provided
   * messages are modified with a new messageId
   *
   ...
   *
   * @param receiptHandle The receipt handle for the message. This is the time based UUID. Receipt
   *     handle also includes the shard number.
   * @throws PersistenceException
   */
  public void deleteMessage(String queueUrl, String receiptHandle) throws PersistenceException;

  /**
   * Receive the next set of messages from the Queue. Receives from random shard and random
   * partition.
   *
   * @param queue The queue which contains the messages.
   * @param receiveAttributes The set of attributes for the message, like the new visibility timeout
   *     and others.
   * @throws PersistenceException
   * @throws IOException
   * @throws NoSuchAlgorithmException
  ...
  /**
   * Peek the queue with the given Queue URL for the next set of messages chosen at random from the
   * queue
   *
   * @param queueUrl The URL of the Queue
   * @param shard The shard to be used
  ...
  /**
   * @throws Get message payload for a list of message-ids
