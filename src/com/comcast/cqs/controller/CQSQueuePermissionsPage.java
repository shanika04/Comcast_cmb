...
public class CQSQueuePermissionsPage extends AdminServletBase {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (params.containsKey("Remove")) {
      ...
      try {
        RemovePermissionRequest removePermissionRequest =
            new RemovePermissionRequest(queueUrl, labelSid);
        sqs.removePermission(removePermissionRequest);
        logger.debug(
            "event=remove_permission queue_url="
                + queueUrl
                + " label="
                + labelSid
                + " user_id="
    ...
    if (user != null) {
      ...
      out.println(
          "<tr><td><b>Queue Name:</b></td><td>"
              + Util.getNameForAbsoluteQueueUrl(queueUrl)
              + "</td></tr>");
    ...
    if (policy != null && !policy.getStatements().isEmpty()) {
      ...
      for (int i = 0; stmts != null && i < stmts.size(); i++) {
        ...
        out.print(
            "<form action=\"/webui/cqsuser/permissions/?userId="
                + user.getUserId()
                + "&queueUrl="
                + queueUrl
                + "\" method=POST>");
        ...
        out.println(
            "<td><input type='submit' value='Remove' name='Remove'><input type='hidden' name='sid' value='"
                ...
                + userId
                + "'><input type='hidden' name='queueName' value='"
                + queueName
                + "'></td></tr></form>");
      }
    ...
    out.println(
        "<p><a href='' onclick=\"window.open('/webui/cqsuser/addpermission/?queueName="
            + queueName
            + "&userId="
            + userId
