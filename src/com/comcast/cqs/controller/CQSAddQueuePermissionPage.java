...
public class CQSAddQueuePermissionPage extends AdminServletBase {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (params.containsKey("Add") && validInput.equals("")) {
        ...
        if (usersList != null
          ...
          try {
            AddPermissionRequest addPermissionRequest =
                new AddPermissionRequest(queueUrl, sid, usersList, actionsList);
            sqs.addPermission(addPermissionRequest);
            logger.debug(
                "event=add_permission queue_url="
                    + queueUrl
                    + " label="
                    + sid
                    + " user_id="
      ...
      out.println(
          "<form action=\"/webui/cqsuser/addpermission/?queueName="
              + queueName
              + "\" method=POST>");
      out.println("<input type='hidden' name='userId' value='" + userId + "'>");
      out.println(
          "<p>Permissions enable you to control which operations a user can perform on a queue.</p>");
      ...
      out.println(
          "<option value='ChangeMessageVisibility'"
              + (actionsList != null && actionsList.contains("ChangeMessageVisibility")
                  ? "selected"
                  : "")
              + ">ChangeMessageVisibility</option>");
      out.println(
          "<option value='GetQueueAttributes'"
              + (actionsList != null && actionsList.contains("GetQueueAttributes")
                  ? "selected"
                  : "")
              + ">GetQueueAttributes</option>");
      out.println(
          "<option value='GetQueueUrl'"
