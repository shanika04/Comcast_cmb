  ...
  public boolean doAction(User user, AsyncContext asyncContext) throws Exception {
    ...
    if (request.getParameter(CQSConstants.MAX_NUMBER_OF_MESSAGES) != null) {
      ...
      int maxNumberOfMessages =
          Integer.parseInt(request.getParameter(CQSConstants.MAX_NUMBER_OF_MESSAGES));
    ...
    if (request.getParameter(CQSConstants.VISIBILITY_TIMEOUT) != null) {
      ...
      int visibilityTimeout =
          Integer.parseInt(request.getParameter(CQSConstants.VISIBILITY_TIMEOUT));
    ...
    List<CQSMessage> messageList =
        PersistenceFactory.getCQSMessagePersistence().receiveMessage(queue, msgParam);
    ...
    if (messageList.size() == 0 && waitTimeSeconds > 0) {
      ...
      logger.debug(
          "event=queueing_context queue_arn="
              + queue.getArn()
              + " wait_time_sec="
              + waitTimeSeconds);

      CQSLongPollReceiver.contextQueues.putIfAbsent(
          queue.getArn(), new ConcurrentLinkedQueue<AsyncContext>());
      ...
      String out =
          CQSMessagePopulator.getReceiveMessageResponseAfterSerializing(
              messageList, filterAttributes, filterMessageAttributes);
