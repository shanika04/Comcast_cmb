  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (parameters.containsKey("Send")) {
      ...
      try {
        ...
        logger.debug(
            "event=delete_message queue_url= " + queueUrl + " receipt_handle=" + receiptHandle);

      } catch (Exception ex) {
        logger.error(
            "event=delete_message queue_url= " + queueUrl + " receipt_handle=" + receiptHandle, ex);
        throw new ServletException(ex);
      }
    ...
    out.println(
        "<form id='frm1' action=\"/webui/cqsuser/message/?userId="
            + userId
            + "&queueName="
            + queueName
            + "\" method=POST>");
    ...
    out.println(
        "<form id='formsendmessage' action=\"/webui/cqsuser/message/?userId="
            + userId
            + "&queueName="
            + queueName
            + "\" method=POST>");
    ...
    if (receivedMessage != null) {
      ...
      if (attributes.get("ApproximateFirstReceiveTimestamp") != null) {
        try {
          timeReceived =
              new Date(Long.parseLong(attributes.get("ApproximateFirstReceiveTimestamp")))
                  .toString();
        } catch (Exception ex) {
        }
      }
      ...
      out.println(
          "<tr><th></th><th>Receipt Handle</th><th>MD5</th><th>Body</th><th>Num Msg Attributes</th><th>Time Sent</th><th>Time First Received (Appr.)</th><th>Receive Count (Appr.)</th><th>Sender</th><th>&nbsp;</th></tr>");
      ...
      out.println(
          "<td><form action=\"/webui/cqsuser/message/?userId="
              + user.getUserId()
              + "&queueName="
              + queueName
              + "&receiptHandle="
    ...
    try {
      ...
      if (queueUrl != null) {
        ...
        String peekRequestUrl =
            cqsServiceBaseUrl
                + user.getUserId()
                + "/"
                + queueName
                + "?Action=PeekMessage&AWSAccessKeyId="
                + user.getAccessKey()
                + "&MaxNumberOfMessages=10&Shard="
                + shard
        ...
        for (Element messageElement : messageElements) {
          ...
          List<Element> attributeElements =
              XmlUtil.getCurrentLevelChildNodes(messageElement, "Attribute");
          ...
          List<Element> messageAttributeElements =
              XmlUtil.getCurrentLevelChildNodes(messageElement, "MessageAttribute");
          Map<String, CQSMessageAttribute> messageAttributes =
              new HashMap<String, CQSMessageAttribute>();
        ...
        String peekRequestFirstMessageUrl =
            cqsServiceBaseUrl
                + user.getUserId()
                + "/"
                + queueName
                + "?Action=PeekMessage&AWSAccessKeyId="
                + user.getAccessKey()
                + "&MaxNumberOfMessages=1&Shard="
                + shard;
        String peekFirstMessageXml =
            httpPOST(cqsServiceBaseUrl, peekRequestFirstMessageUrl, awsCredentials);
        Element rootFirstMessage = XmlUtil.buildDoc(peekFirstMessageXml);

        List<Element> messageElementsForFirstMessage =
            XmlUtil.getCurrentLevelChildNodes(
                XmlUtil.getCurrentLevelChildNodes(rootFirstMessage, "ReceiveMessageResult").get(0),
                "Message");
        if (messageElementsForFirstMessage.size() == 1) {
          queueFirstMessageHandle =
              XmlUtil.getCurrentLevelTextValue(
                      messageElementsForFirstMessage.get(0), "ReceiptHandle")
                  .trim();
        }
      }
    ...
    for (int i = 0; availableMessages != null && i < availableMessages.size(); i++) {
      ...
      if ((messageBody != null) && (messageBody.length() > 300)) {
        ...
        out.println(messageBodyPart1);
        out.println(
            "<div id='detail" + i + "' style=\"display: none;\">" + messageBodyPart2 + "</div>");
        out.println(
            "<input type=button name=type id='bt"
                + i
                + "' value='More' onclick=\"setVisibility('detail"
                + i
                + "', 'bt"
                + i
                + "');\";> ");
      ...
      out.println(
          "<td>"
              + (message.getMessageAttributes() == null ? 0 : message.getMessageAttributes().size())
              + "</td>");
      out.println("<td>" + timeSent + "</td>");
      out.println("<td>" + timeReceived + "</td>");
      out.println("<td>" + attributes.get("ApproximateReceiveCount") + "</td>");
    ...
    if (prevHandle != null) {
      if ((previousHandle) != null) {
        out.println(
            "<a style='float:left;' href='/webui/cqsuser/message/?userId="
                + user.getUserId()
                + "&queueName="
                + queueName
                + "&nextHandle="
    ...
    } else if ((previousHandle != null)
        && (!previousHandle.equals(queueFirstMessageHandle))) { // this is for all other cases
      out.println(
          "<a style='float:left;' href='/webui/cqsuser/message/?userId="
              + user.getUserId()
              + "&queueName="
    ...
    if (availableMessages != null && availableMessages.size() > 0) {
      out.println(
          "<a style='float:right;' href='/webui/cqsuser/message/?userId="
              + user.getUserId()
              + "&queueName="
              + queueName
              + "&prevHandle="
