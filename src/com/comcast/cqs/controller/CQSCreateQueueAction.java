...
public class CQSCreateQueueAction extends CQSAction {
  ...
  public boolean doAction(User user, AsyncContext asyncContext) throws Exception {
    ...
    CQSQueue existingQueue =
        PersistenceFactory.getQueuePersistence().getQueue(newQueue.getRelativeUrl());
    ...
    while (attributeName != null) {
      ...
      if (attributeName.equals(CQSConstants.VISIBILITY_TIMEOUT)) {
        ...
        if (visibilityTo < 0
            || visibilityTo > CMBProperties.getInstance().getCQSMaxVisibilityTimeOut()) {
          throw new CMBException(
              CMBErrorCodes.InvalidParameterValue,
        ...
        if (maxMessageSize < 0
            || maxMessageSize > CMBProperties.getInstance().getCQSMaxMessageSize()) {
          throw new CMBException(
              CMBErrorCodes.InvalidParameterValue,
              CQSConstants.MAXIMUM_MESSAGE_SIZE
                  + " should be between 0 and "
        ...
        if (existingQueue != null
            && existingQueue.getMsgRetentionPeriod() != messageRetentionPeriod) {
          throwQueueExistsError = true;
          break;
        }
        ...
        if (existingQueue != null
            && existingQueue.getDelaySeconds() != Integer.parseInt(attributeValue)) {
          throwQueueExistsError = true;
          break;
        }
        ...
        if (existingQueue != null
            && existingQueue.getReceiveMessageWaitTimeSeconds()
        ...
        // number of Cassandra rows used to store messages, default is 100 - this number can be set
        ...
        // number of internal queues used for random sharding to trade message ordering for enhanced
        // single-queue
        // throughput - this number can only be set at queue creation time and cannot be changed
        // later

        if (existingQueue != null
            && existingQueue.getNumberOfShards() != Integer.parseInt(attributeValue)) {
          throwQueueExistsError = true;
          break;
        }
    ...
    if (throwQueueExistsError) {
      throw new CMBException(
          CQSErrorCodes.QueueNameExists, "Queue name with " + queueName + " exists");
    }
    ...
    for (int shard = 0; shard < numberOfShards; shard++) {
      PersistenceFactory.getCQSMessagePersistence()
          .checkCacheConsistency(newQueue.getRelativeUrl(), shard, false);
    }
