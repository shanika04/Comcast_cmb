...
public class CQSMessagePopulator extends CQSPopulator {
  ...
  public static String getSendMessageResponse(CQSMessage message) {
    ...
    out.append("\t\t<MD5OfMessageBody>")
        .append(message.getMD5OfBody())
        .append("</MD5OfMessageBody>\n");
    if (message.getMD5OfMessageAttributes() != null) {
      out.append("\t\t<MD5OfMessageAttributes>")
          .append(message.getMD5OfMessageAttributes())
          .append("</MD5OfMessageAttributes>\n");
    }
  ...
  public static String getSendMessageBatchResponse(
      List<CQSMessage> messages, List<CQSBatchResultErrorEntry> errorList) {
  ...
  public static String getDeleteMessageBatchResponse(
      List<String> ids, List<CQSBatchResultErrorEntry> errorList) {
  ...
  public static String getChangeMessageVisibilityBatchResponse(
      List<String> ids, List<CQSBatchResultErrorEntry> errorList) {
  ...
  public static String getReceiveMessageResponseAfterSerializing(
      List<CQSMessage> messages,
      List<String> filterAttributs,
      List<String> filterMessageAttributes) {
  ...
  public static String serializeMessage(
      CQSMessage message, List<String> filterAttributes, List<String> filterMessageAttributes) {
    ...
    messageXml
        ...
        .append(message.getReceiptHandle())
        .append("</ReceiptHandle>\n");
    messageXml.append("\t\t\t<MD5OfBody>").append(message.getMD5OfBody()).append("</MD5OfBody>\n");
    messageXml
        .append("\t\t\t<Body>")
        .append(StringEscapeUtils.escapeXml(message.getBody()).replaceAll("\r", "&#xD;"))
        .append("</Body>\n");
    ...
    if (message.getMessageAttributes() != null && message.getMessageAttributes().size() > 0) {
      for (String key : message.getMessageAttributes().keySet()) {
        if (filterMessageAttributes.contains("All")
            || filterMessageAttributes.contains("all")
            || filterMessageAttributes.contains(key)) {
  ...
  private static StringBuffer fillAttributesInReturn(
      CQSMessage message, List<String> filterAttributes) {
    ...
    if (message.getAttributes() != null) {
      for (Map.Entry<String, String> entry : message.getAttributes().entrySet()) {
        if (entry.getKey() != null
            ...
            && !entry.getKey().equals(CQSConstants.DELAY_SECONDS)
            && (filterAttributes.isEmpty()
                || filterAttributes.contains("All")
                || filterAttributes.contains(entry.getKey()))) {
          attributesXmlFragment
              .append("\t\t\t<Attribute>")
              .append("<Name>")
              .append(entry.getKey())
              .append("</Name>")
