...
public abstract class AdminServletBase extends HttpServlet {
  ...
  protected boolean redirectUnauthenticatedUser(
  ...
  protected void simpleHeader(HttpServletRequest request, PrintWriter out, String title)
      throws ServletException {
    ...
    try {
      ...
      BufferedReader br =
          new BufferedReader(new FileReader(getServletContext().getRealPath("WEB-INF/global.css")));
      String line;
   ...
   * @throws IOException
   */
  protected void header(HttpServletRequest request, PrintWriter out, String title)
      throws ServletException {
    ...
    if (isAuthenticated(request)) {
      ...
      out.println(
          "<table width='100%' border='0'><tr><td width='100%' align='left'>Welcome "
      ...
      if (isAdmin(request)) {
        ...
        User mainUser = getAuthenticatedUser(request);
        out.println(
            "<a href='/webui/cnsuser?userId=" + mainUser.getUserId() + "'> Topics</a>" + " | ");
        out.println(
            "<a href='/webui/cqsuser?userId=" + mainUser.getUserId() + "'> Queues</a>" + " | ");
      }
  ...
  protected String httpPOST(String baseUrl, String urlString, AWSCredentials awsCredentials) {
    ...
    try {
      ...
      Request<CreateQueueRequest> request =
          new CreateQueueRequestMarshaller().marshall(createQueueRequest);
      ...
      for (int i = 0; i < parameterArray.length; i++) {
        requestParameters.put(
            parameterArray[i].substring(0, parameterArray[i].indexOf("=")),
