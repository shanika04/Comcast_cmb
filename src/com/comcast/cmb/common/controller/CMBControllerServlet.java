 ...
 */
public abstract class CMBControllerServlet extends HttpServlet {
  ...
  /**
   * This instance of the acccumulator is used throughout the code to accumulate response times for
   * a particular thread
   */
  public static final ValueAccumulator valueAccumulator = new ValueAccumulator();

  public static final String VERSION = "2.2.46";

  public static final int HARD_TIMEOUT_SEC = CMBProperties.getInstance().getCMBRequestTimeoutSec();

  public static volatile ConcurrentHashMap<String, AtomicLong> callStats;
  public static volatile ConcurrentHashMap<String, AtomicLong> callFailureStats;

  public static final int NUM_MINUTES = 60;
  public static final int NUM_BUCKETS = 10;

  public static volatile AtomicLong[][] callResponseTimes1MS =
      new AtomicLong[NUM_MINUTES][NUM_BUCKETS];
  public static volatile AtomicLong[][] callResponseTimes10MS =
      new AtomicLong[NUM_MINUTES][NUM_BUCKETS];
  public static volatile AtomicLong[][] callResponseTimes100MS =
      new AtomicLong[NUM_MINUTES][NUM_BUCKETS];
  public static volatile AtomicLong[][] callResponseTimes1000MS =
      new AtomicLong[NUM_MINUTES][NUM_BUCKETS];
  public static volatile AtomicLong[][] callResponseTimesRedisMS =
      new AtomicLong[NUM_MINUTES][NUM_BUCKETS];
  public static volatile AtomicLong[][] callResponseTimesCassandraMS =
      new AtomicLong[NUM_MINUTES][NUM_BUCKETS];

  public static volatile ConcurrentHashMap<String, AtomicLong[][]> callResponseTimesByApi;

  public static volatile AtomicInteger currentMinute;

  public static volatile String[] recentErrors = new String[10];
  public static volatile int recentErrorIdx = -1;
  ...
  public void init() throws ServletException {
    ...
    try {
      ...
      if (!initialized) {
        ...
        workerPool =
            new ScheduledThreadPoolExecutor(CMBProperties.getInstance().getCMBWorkerPoolSize());
        initStats();
        initialized = true;
      }
   ...
   * @throws Exception
   */
  protected abstract boolean handleAction(String action, User user, AsyncContext asyncContext)
      throws Exception;
   ...
   */
  protected abstract boolean isValidAction(String action) throws ServletException;

  /**
   * @param action
   * @return true if validation is required. False otherwise Should be overriden by subclasses
   */
    ...
    while (keys.hasMoreElements()) {
      ...
      if (key.equals(CQSConstants.MESSAGE_BODY)) {
        if (CMBProperties.getInstance().getMaxMessagePayloadLogLength() > 0) {
          if (value != null
              && value.length() > CMBProperties.getInstance().getMaxMessagePayloadLogLength()) {
            value =
                value.substring(0, CMBProperties.getInstance().getMaxMessagePayloadLogLength())
                    + "...";
  ...
  private void logStats(
      String action, long responseTimeMS, long redisTimeMS, long cassandraTimeMS) {
  ...
  private String getLogLine(
    ...
    logLine.append(
        ((this instanceof CQSControllerServlet) ? (" queue=" + request.getPathInfo()) : ""));
    ...
    if (request.getAttribute("lp") == null) {
      ...
      if ((status != null) && (!status.equals("timeout"))) {
        ...
        logLine.append(
            " cass_num_rd=" + valueAccumulator.getCounter(AccumulatorName.CassandraRead));
        logLine.append(
            " cass_num_wr=" + valueAccumulator.getCounter(AccumulatorName.CassandraWrite));
        logLine.append(
            ((this instanceof CNSControllerServlet)
                ? (" cnscqs_ms="
                    + CMBControllerServlet.valueAccumulator.getCounter(AccumulatorName.CNSCQSTime))
                : ""));
        logLine.append(
            ((this instanceof CQSControllerServlet)
        ...
        logLine.append(
            " auth_ms="
                + valueAccumulator.getCounter(AccumulatorName.CMBControllerPreHandleAction));
      }
      ...
      logLine
          .append(" lp_ms=")
          .append(System.currentTimeMillis() - request.getRequestReceivedTimestamp());
    ...
    // logLine.append("
    // async_pool_queue=").append(CMBControllerServlet.workerPool.getQueue().size()).
  ...
  private String getFullRequestUrl(AsyncContext asyncContext) {
    HttpServletRequest request = (HttpServletRequest) asyncContext.getRequest();
    return "event=raw_request url="
        + request.getRequestURL()
        + "/"
        + this.getUrlQueryString(asyncContext);
  }
  ...
  private void handleRequest(AsyncContext asyncContext) throws ServletException, IOException {
    ...
    try {
      ...
      valueAccumulator.addToCounter(
          AccumulatorName.AsyncQueueTime,
      ...
      if (!isValidAction(action)) {
        ...
        String errXml =
            createErrorResponse(
                CMBErrorCodes.InvalidAction.getCMBCode(), action + " is not a valid action");
      ...
      if (CMBProperties.getInstance().isCMBStatsEnabled()) {
        logStats(
            action,
            ts2 - ts1,
            valueAccumulator.getCounter(AccumulatorName.RedisTime),
      ...
      if (CMBProperties.getInstance().isCMBStatsEnabled() && action != null && !action.equals("")) {
        ...
        StringBuffer errorDetail =
            new StringBuffer(new Date() + "|" + logLine + "|" + ex.getMessage() + "\n");
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (actionParam != null
        && actionParam.equals("ReceiveMessage")
        && waitTimeSecondsParam != null) {
      ...
      try {
        ...
        if (waitTimeSeconds >= 1
            && waitTimeSeconds <= CMBProperties.getInstance().getCMBRequestTimeoutSec()) {
      ...
      String queueUrl =
          com.comcast.cqs.util.Util.getRelativeForAbsoluteQueueUrl(
      ...
      if (waitTimeSeconds > 0) {
        ...
        logger.debug(
            "event=set_default_timeout secs="
                + CMBProperties.getInstance().getCMBRequestTimeoutSec());
      }
    ...
    asyncContext.addListener(
        new AsyncListener() {
          ...
          public void onComplete(AsyncEvent asyncEvent) throws IOException {
            ...
            if (action != null
                && action.equals("ReceiveMessage")
                && request.getAttribute("lp") != null) {
              ...
              if (lp_ms != null) {
                logLine =
                    getLogLine(
                        asyncContext, request, user, (Long) request.getAttribute("lp_ms"), "ok");
              } else {
          ...
          public void onError(AsyncEvent asyncEvent) throws IOException {
            ...
            if (queue != null) {
              ...
              ConcurrentLinkedQueue<AsyncContext> queueContextsList =
                  CQSLongPollReceiver.contextQueues.get(queue.getArn());
              ...
              logger.error(
                  "event=on_timeout "
                      + getLogLine(
                          asyncContext,
                          request,
                          authModule.getUserByRequest(request),
                          0,
                          "timeout"));
            }
        ...
        new Runnable() {
          ...
          public void run() {
            ...
            try {
              ...
              logger.error(
                  "event=async_api_call_failure receipt_id=" + ReceiptModule.getReceiptId(), ex);
            }
  ...
  public static String createErrorResponse(String code, String errorMsg) {
    StringBuffer message =
        new StringBuffer("<ErrorResponse>\n")
            .append("\t<Error>\n")
            ...
            .append("\t</Error>\n")
            .append("\t<RequestId>")
            .append(ReceiptModule.getReceiptId())
            .append("</RequestId>\n")
