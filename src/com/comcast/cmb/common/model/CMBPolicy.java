...
public class CMBPolicy {
  ...
  public static final List<String> STATEMENT_ATTRIBUTES =
      Arrays.asList("Sid", "Effect", "Principal", "Action", "Resource", "Condition");
  ...
  public static final List<String> CQS_ACTIONS =
      Arrays.asList(
          "AddPermission",
          ...
          "SendMessageBatch",
          "SetQueueAttributes");
  public static final List<String> CNS_ACTIONS =
      Arrays.asList(
          "AddPermission",
          "ConfirmSubscription",
          "CreateTopic",
  ...
  public boolean addStatement(
    ...
    for (String action : actionList) {
      ...
      if (!CNS_ACTIONS.contains(action) && !CQS_ACTIONS.contains(action) && !action.equals("*")) {
        throw new CMBException(
            CMBErrorCodes.InvalidParameterValue, "Invalid action parameter " + action);
      }
    ...
    this.statements.add(
        new CMBStatement(sid, effect, userList, normalizedActionList, resource, condition));
  ...
  /**
   * check all statements matching user/action, return false upon the first Deny effect or no Allow
   * effect; otherwise return true.
   *
   * @param user
   * @param action
   * @return
  ...
  public void fromString(String policyString) throws Exception {
    ...
    while (policyIterator.hasNext()) {
      ...
      if (!CMBPolicy.POLICY_ATTRIBUTES.contains(policyAttribute)) {
        throw new CMBException(
            CMBErrorCodes.InvalidAttributeValue, "Invalid value for the parameter Policy");
      }
    }
