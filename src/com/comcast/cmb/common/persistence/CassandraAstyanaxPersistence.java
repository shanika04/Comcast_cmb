...
public class CassandraAstyanaxPersistence extends AbstractDurablePersistence {
  ...
  private void initPersistence() {
    ...
    for (String k : keyspaceNames) {
      ...
      ConnectionPoolConfigurationImpl connectionPoolConfiguration =
          new ConnectionPoolConfigurationImpl("CMBAstyananxConnectionPool")
      ...
      if (username != null && password != null) {
        connectionPoolConfiguration.setAuthenticationCredentials(
            new SimpleAuthenticationCredentials(username, password));
      }
                  ...
                  new AstyanaxConfigurationImpl()
                      ...
                      .setDefaultReadConsistencyLevel(
                          ConsistencyLevel.valueOf(
                              "CL_" + CMBProperties.getInstance().getReadConsistencyLevel()))
                      .setDefaultWriteConsistencyLevel(
                          ConsistencyLevel.valueOf(
                              "CL_" + CMBProperties.getInstance().getWriteConsistencyLevel())))
  ...
  private <K, N, V> List<CmbRow<K, N, V>> getRows(List<Row<K, N>> rows)
  ...
  private static ConcurrentHashMap<String, ColumnFamily> cf =
      new ConcurrentHashMap<String, ColumnFamily>();

  static {
    ColumnFamily<String, String> CF_CNS_TOPICS =
        new ColumnFamily<String, String>(
            CNS_TOPICS, StringSerializer.get(), StringSerializer.get());
    cf.put(CNS_TOPICS, CF_CNS_TOPICS);

    ColumnFamily<String, String> CF_CNS_TOPICS_BY_USER_ID =
        new ColumnFamily<String, String>(
            CNS_TOPICS_BY_USER_ID, StringSerializer.get(), StringSerializer.get());
    ...
    ColumnFamily<String, String> CF_CNS_TOPIC_ATTRIBUTES =
        new ColumnFamily<String, String>(
            CNS_TOPIC_ATTRIBUTES, StringSerializer.get(), StringSerializer.get());
    ...
    ColumnFamily<String, String> CF_CQS_API_SERVERS =
        new ColumnFamily<String, String>(
            CQS_API_SERVERS, StringSerializer.get(), StringSerializer.get());
    cf.put(CQS_API_SERVERS, CF_CQS_API_SERVERS);

    ColumnFamily<String, String> CF_CMB_USERS =
        new ColumnFamily<String, String>(CMB_USERS, StringSerializer.get(), StringSerializer.get());
  ...
  public <K, N, V> List<CmbRow<K, N, V>> readAllRows(
    ...
    long ts1 = System.currentTimeMillis();
    logger.debug(
        "event=read_next_n_non_empty_rows cf="
            + columnFamily
            + " num_rows="
            + numRows
            + " num_cols="
    ...
    try {
      ...
      if (whereColumn != null && whereValue != null) {
        query
            ...
            .equals()
            .value(whereValue, getSerializer(valueSerializer));
      }
      ...
      if (columnValues != null) {
        for (N columnName : columnValues.keySet()) {
          query
              ...
              .equals()
              .value(columnValues.get(columnName), getSerializer(valueSerializer));
        }
      }
    ...
    try {
      ...
      RowQuery<K, N> rq =
          getKeyspace(keyspace)
              ...
              .getKey(key)
              .withColumnRange(
                  getComposite(firstColumnName), getComposite(lastColumnName), false, numCols);
  ...
  public <K, N, V> void insertRow(
    ...
    try {
      MutationBatch m = getKeyspace(keyspace).prepareMutationBatch();
      ColumnListMutation<N> clm =
          m.withRow((ColumnFamily<K, N>) getColumnFamily(columnFamily), rowKey);
      for (N columnName : columnValues.keySet()) {
        clm.putColumn(
            (N) getComposite(columnName),
            columnValues.get(columnName),
    ...
    try {
      ...
      for (K rowKey : rowColumnValues.keySet()) {
        ...
        for (N columnName : rowColumnValues.get(rowKey).keySet()) {
          clm.putColumn(
              (N) getComposite(columnName),
              rowColumnValues.get(rowKey).get(columnName),
              getSerializer(valueSerializer),
              ttl);
          CMBControllerServlet.valueAccumulator.addToCounter(AccumulatorName.CassandraWrite, 1L);
        }
  ...
  public <K, N> void delete(
    ...
    long ts1 = System.currentTimeMillis();
    logger.debug(
        "event=delete column_family=" + columnFamily + " key=" + key + " column=" + column);
  ...
  public <K, N> void deleteBatch(
    ...
    try {
      ...
      if (columnList == null || columnList.isEmpty()) {
        for (K k : keyList) {
          ColumnListMutation<N> clm =
              m.withRow((ColumnFamily<K, N>) getColumnFamily(columnFamily), k);
          clm.delete();
          CMBControllerServlet.valueAccumulator.addToCounter(AccumulatorName.CassandraWrite, 1L);
        }
      } else {
        ...
        for (int i = 0; i < keyList.size(); i++) {
          ColumnListMutation<N> clm =
              m.withRow((ColumnFamily<K, N>) getColumnFamily(columnFamily), keyList.get(i));
