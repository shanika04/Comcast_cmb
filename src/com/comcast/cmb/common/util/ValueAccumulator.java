...
/**
 * This class helps accumulate long values associated with various keys per given VM process This
 * class is mostly used for accumulating db time to serve a single request.
 *
...
public final class ValueAccumulator {
  ...
  private final ThreadLocal<long[]> keyToValue =
  ...
  public void initializeCounters(AccumulatorName... keys) {
    for (AccumulatorName key : keys) {
      if (keyToValue.get()[key.ordinal()] != -1L) {
        // logger.warn("initializeCounter called but on an in-use counter. Please call
        // deleteCounter()");
      }
  ...
  public void addToCounter(AccumulatorName key, long ammount) {
    ...
    if (val == -1L) {
      // logger.warn("Must initialize Accumulator before adding to counter with key=" + key + ", not
      // adding to the counter the new ammount=" + ammount);
      return;
    }
  ...
  /**
   * Clear the counter for this key. Must be called when we are done with one HTTP request
   *
   * @param key
   */
