...
/**
 * This class is a highly concurrent cache used to cache key, value pairs with an expiration on the
 * value. The class is thread-safe K - the key type V - the Value type
 *
 * @author aseem Class ia thread-safe
 */
public final class ExpiringCache<K, V> {
  ...
  /**
   * Clean up the expired key values. This operation has complexity O(n) where n is the number of
   * key, value pairs stored in the cache. Note: THis method is not synchronized with the
   * getAndSetIfNotPresent. Its possible for a race condition where we evict something from the
   * cache that was just recently set in cache. That's an acceptable tradeoff This is why we try and
   * minimize calls to cleanup
   */
  ...
  public boolean containsKey(K key) {
    ...
    if (existingValContainer != null) {
      if (existingValContainer.createdTimestamp + existingValContainer.exp
  ...
  /**
   * @param key THe key
   * @param valueGetter The Caller that will get the V value if none is cached or if previous one
   *     expired
   * @param exp THe expiration time in milliseconds
   * @return The Value V that was cached ir that just got cached.
   * @throws InterruptedException
   * @throws ExecutionException Note: method will block if we need to call valueGetter Note: THere
   *     is a window where t1 could add a new future-task and t2 could be removing it
   */
  public V getAndSetIfNotPresent(K key, Callable<V> valueGetter, int exp)
    ...
    try {
      ...
      throw new IllegalStateException(
          "Could not get value from user passed Callable:" + valueGetter, e);
    }
  }
}
