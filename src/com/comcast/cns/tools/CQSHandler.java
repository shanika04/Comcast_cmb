...
public class CQSHandler {
  ...
  private static synchronized void initialize() throws PersistenceException {
    ...
    if (CMBProperties.getInstance().getCNSUserAccessKey() != null
      ...
      awsCredentials =
          new BasicAWSCredentials(
              CMBProperties.getInstance().getCNSUserAccessKey(),
      ...
      awsCredentials =
          new BasicAWSCredentials(cnsInternal.getAccessKey(), cnsInternal.getAccessSecret());
    }
    ...
    useInlineApiCalls =
        CMBProperties.getInstance().useInlineApiCalls()
  ...
  public static void changeMessageVisibility(
      String relativeQueueUrl, String receiptHandle, int visibilityTimeout) throws Exception {

    long ts1 = System.currentTimeMillis();

    if (useInlineApiCalls) {
      CQSAPI.changeMessageVisibility(
          cnsInternal.getUserId(), relativeQueueUrl, receiptHandle, visibilityTimeout);
    } else {
    ...
    logger.debug(
        "event=change_message_visibility receipt_handle="
            + receiptHandle
            + " vto="
            + visibilityTimeout);
  }

  public static List<CQSMessage> receiveMessage(
      String relativeQueueUrl, int waitTimeSeconds, int maxNumberOfMessages) throws Exception {
    ...
    if (useInlineApiCalls && waitTimeSeconds == 0) {
      ...
      messages =
          CQSAPI.receiveMessages(
              cnsInternal.getUserId(), relativeQueueUrl, maxNumberOfMessages, null);
  ...
  public static String getQueueUrl(String queueName) throws Exception {
    ...
    CQSQueue queue =
        CQSCache.getCachedQueue(
            Util.getRelativeQueueUrlForName(queueName, cnsInternal.getUserId()));
  ...
  public static String sendMessage(
      String relativeQueueUrl, String message, Map<String, CQSMessageAttribute> messageAttributes)
      throws Exception {
    ...
    if (useInlineApiCalls) {
      receiptHandle =
          CQSAPI.sendMessage(
              cnsInternal.getUserId(), relativeQueueUrl, message, null, messageAttributes);
    } else {
      String absoluteQueueUrl = Util.getAbsoluteQueueUrlForRelativeUrl(relativeQueueUrl);
        ...
        if (ex.getStatusCode() == 400) {
          ...
          if (createQueueResponse.getQueueUrl() == null) {
            throw new IllegalStateException(
                "Could not create queue with name " + queueNamePrefix + i);
          }

          logger.info(
              "event=created_missing_queue name="
                  + queueNamePrefix
                  + i
                  + " url="
  ...
  public static boolean doesQueueNotExist(Exception ex) {
    ...
    if (ex instanceof AmazonServiceException
        && ((AmazonServiceException) ex).getStatusCode() == 400) {
      doesNotExist = true;
