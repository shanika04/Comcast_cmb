...
public class CNSEndpointPublisherJobProducer implements CNSPublisherPartitionRunnable {
  ...
  private static final String CNS_PRODUCER_QUEUE_NAME_PREFIX =
      CMBProperties.getInstance().getCNSPublishQueueNamePrefix();

  private static volatile boolean initialized = false;

  private static volatile ICNSSubscriptionPersistence subscriptionPersistence =
  ...
  @Override
  /**
   * 1. Call ReceiveMessage(PublishJobQ.<n> where n in [0..numPublishJobQs] 2. if no message, go to
   * sleep for 100ms and go back to 1 3. if message found, 3.1 read sub-list for topic, 3.2
   * partition them by batch size 3.3 Enqueue into EndpointPublishJobQ.<m> where m is randomly
   * selected between [0..numEPPublishJobQs] 3.4 go back to 1
   */
    ...
    try {
      ...
      if (msg != null) {
        ...
        if (messageExpirationSeconds != 0
            && System.currentTimeMillis() - publishMessage.getTimestamp().getTime()
                > messageExpirationSeconds * 1000) {
          logger.error(
              "event=deleting_publish_job reason=message_too_old topic_arn="
        ...
        try {
          ...
          logger.error(
              "event=deleting_publish_job reason=topic_not_found topic_arn="
                  + publishMessage.getTopicArn());
          CQSHandler.deleteMessage(queueUrl, msg.getReceiptHandle());
          CMBControllerServlet.valueAccumulator.deleteAllCounters();
          return true; // return true to avoid backoff
        ...
        if (subscriptions != null && subscriptions.size() > 0) {
          ...
          messageFound = true;
          List<CNSEndpointPublishJob> epPublishJobs =
              createEndpointPublishJobs(publishMessage, subscriptions);
          ...
          for (CNSEndpointPublishJob epPublishJob : epPublishJobs) {
            ...
            CQSHandler.sendMessage(
                epQueueUrl, epPublishJob.serialize(), msg.getMessageAttributes());
          }
        }
        ...
        logger.debug(
            "event=processed_producer_job cns_cqs_ms="
      ...
      CQSHandler.ensureQueuesExist(
          CNS_PRODUCER_QUEUE_NAME_PREFIX, CMBProperties.getInstance().getCNSNumPublishJobQueues());
    }
  ...
  public static List<CNSEndpointPublishJob.CNSEndpointSubscriptionInfo> getSubscriptionsForTopic(
      String topicArn) throws Exception {

    List<CNSEndpointPublishJob.CNSEndpointSubscriptionInfo> subInfoList =
        new ArrayList<CNSEndpointPublishJob.CNSEndpointSubscriptionInfo>();
    ...
    if (CMBProperties.getInstance().isCNSUseSubInfoCache()) {
      ...
      while (true) {
        ...
        List<CNSSubscription> subscriptions =
            subscriptionPersistence.listSubscriptionsByTopic(nextToken, topicArn, null, 1000);
        ...
        for (CNSSubscription subscription : subscriptions) {
          ...
          if (!subscription.getArn().equals("PendingConfirmation")) {
            ...
            subInfoList.add(
                new CNSEndpointPublishJob.CNSEndpointSubscriptionInfo(
  ...
  public static List<CNSEndpointPublishJob> createEndpointPublishJobs(
      CNSMessage message, List<CNSEndpointPublishJob.CNSEndpointSubscriptionInfo> subscriptions) {

    List<CNSEndpointPublishJob> epPublishJobs = new ArrayList<CNSEndpointPublishJob>();

    if (subscriptions != null) {

      int maxSubsPerEPPublishJob =
          CMBProperties.getInstance().getCNSMaxSubscriptionsPerEndpointPublishJob();
      int numEPPublishJobs = (int) Math.ceil(subscriptions.size() / (float) maxSubsPerEPPublishJob);
      int subIndex = 0;

      for (int i = 0; i < numEPPublishJobs; i++) {

        List<CNSEndpointPublishJob.CNSEndpointSubscriptionInfo> epPublishJobSubscriptions =
            new ArrayList<CNSEndpointPublishJob.CNSEndpointSubscriptionInfo>();
        ...
        for (int j = 0; j < maxSubsPerEPPublishJob; j++) {
          ...
          if (CMBProperties.getInstance().isCNSUseSubInfoCache()) {
            subInfo =
                new CNSCachedEndpointPublishJob.CNSCachedEndpointSubscriptionInfo(
                    subscriptions.get(subIndex).protocol,
                    subscriptions.get(subIndex).endpoint,
                    subscriptions.get(subIndex).subArn,
                    subscriptions.get(subIndex).rawDelivery);
          } else {
            subInfo =
                new CNSEndpointSubscriptionInfo(
                    subscriptions.get(subIndex).protocol,
                    subscriptions.get(subIndex).endpoint,
                    subscriptions.get(subIndex).subArn,
                    subscriptions.get(subIndex).rawDelivery);
