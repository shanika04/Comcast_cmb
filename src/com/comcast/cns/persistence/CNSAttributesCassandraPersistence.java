...
public class CNSAttributesCassandraPersistence implements ICNSAttributesPersistence {
  ...
  @Override
  public void setTopicAttributes(CNSTopicAttributes topicAttributes, String topicArn)
      throws Exception {
    ...
    cassandraHandler.insertRow(
        ...
        getColumnValues(topicAttributes),
    ...
    if (topicAttributes.getDisplayName() != null) {
      PersistenceFactory.getTopicPersistence()
          .updateTopicDisplayName(topicArn, topicAttributes.getDisplayName());
    }
  ...
  private Map<String, String> getColumnValues(CNSTopicAttributes topicAttributes) {
    ...
    // currently only accept delivery policy parameter, fill up with defaults if elements are
    // missing and return as both delivery policy and effective delivery policy
  ...
  public CNSTopicAttributes getTopicAttributes(String topicArn) throws Exception {
    ...
    if (slice != null) {
      ...
      topicAttributes.setDisplayName(
          PersistenceFactory.getTopicPersistence().getTopic(topicArn).getDisplayName());
    }

    long subscriptionConfirmedCount =
  ...
  public void setSubscriptionAttributes(
      CNSSubscriptionAttributes subscriptionAtributes, String subscriptionArn) throws Exception {

    cassandraHandler.insertRow(
        AbstractDurablePersistence.CNS_KEYSPACE,
        subscriptionArn,
        columnFamilySubscriptionAttributes,
        getColumnValues(subscriptionAtributes),
        CMB_SERIALIZER.STRING_SERIALIZER,
        CMB_SERIALIZER.STRING_SERIALIZER,
        CMB_SERIALIZER.STRING_SERIALIZER,
        null);
  ...
  @Override
  public CNSSubscriptionAttributes getSubscriptionAttributes(String subscriptionArn)
      throws Exception {
    ...
    if (slice != null) {
      ...
      if (slice.getColumnByName("deliveryPolicy") != null) {
        subscriptionAttributes.setDeliveryPolicy(
            new CNSSubscriptionDeliveryPolicy(
                new JSONObject(slice.getColumnByName("deliveryPolicy").getValue())));
      }

      // if "ignore subscription override" is checked, get effective delivery policy from topic
      // delivery policy, otherwise
      ...
      if (topicAttributes != null) {
        ...
        CNSTopicDeliveryPolicy topicEffectiveDeliveryPolicy =
        ...
        if (topicEffectiveDeliveryPolicy != null) {
          ...
          if (topicEffectiveDeliveryPolicy.isDisableSubscriptionOverrides()
            ...
            CNSSubscriptionDeliveryPolicy effectiveDeliveryPolicy =
                new CNSSubscriptionDeliveryPolicy();
            effectiveDeliveryPolicy.setHealthyRetryPolicy(
                topicEffectiveDeliveryPolicy.getDefaultHealthyRetryPolicy());
            effectiveDeliveryPolicy.setSicklyRetryPolicy(
                topicEffectiveDeliveryPolicy.getDefaultSicklyRetryPolicy());
            effectiveDeliveryPolicy.setThrottlePolicy(
                topicEffectiveDeliveryPolicy.getDefaultThrottlePolicy());
            subscriptionAttributes.setEffectiveDeliveryPolicy(effectiveDeliveryPolicy);
          } else {
            subscriptionAttributes.setEffectiveDeliveryPolicy(
                subscriptionAttributes.getDeliveryPolicy());
          }
        }
      }
