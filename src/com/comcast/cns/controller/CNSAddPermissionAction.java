...
public class CNSAddPermissionAction extends CNSAction {
  ...
  public boolean doAction(User user, AsyncContext asyncContext) throws Exception {
    ...
    while (userId != null) {
      ...
      if (userId.equals("*")
          || userHandler.getUserById(userId) != null) { // only add user if they exist
        userList.add(userId);
      }
    ...
    while (action != null) {
      ...
      if (!CMBPolicy.CNS_ACTIONS.contains(action) && !action.equals("*")) {
        throw new CMBException(
            CNSErrorCodes.CNS_InvalidParameter, "Invalid action parameter " + action);
      }
    ...
    if (policy.addStatement(
        CMBPolicy.SERVICE.CNS, label, "Allow", userList, actionList, topicArn, null)) {
