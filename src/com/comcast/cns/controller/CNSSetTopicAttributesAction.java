...
public class CNSSetTopicAttributesAction extends CNSAction {
  ...
  public boolean doAction(User user, AsyncContext asyncContext) throws Exception {
    ...
    if ((userId == null)
        ...
        || (attributeValue == null)) {
      logger.error(
          "event=cns_set_topic_attributes error_code=InvalidParameters attribute_name="
              + attributeName
              + " attribute_value="
              + attributeValue
              + " topic_arn="
    ...
    if (attributeName.equals("DeliveryPolicy")) {
      ...
      logger.debug(
          "event=cns_set_topic_delivery_policy topic_arn="
              + topicArn
              + " value="
      ...
      logger.debug(
          "event=cns_set_topic_policy topic_arn="
              + topicArn
              + "  value="
              + topicAttributes.getPolicy());
    ...
    logger.debug(
        "event=cns_set_topic_attributes attribute_name="
            + attributeName
            + " attribute_value="
            + attributeValue
            + " topic_arn="
