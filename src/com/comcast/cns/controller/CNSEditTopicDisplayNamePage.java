    ...
    if (parameters.containsKey("Edit")) {
      ...
      try {
        SetTopicAttributesRequest setTopicAttributesRequest =
            new SetTopicAttributesRequest(topicArn, "DisplayName", displayName);
        sns.setTopicAttributes(setTopicAttributesRequest);
        logger.debug("event=update_display_name topic_arn=" + topicArn + " user_id= " + userId);
      } catch (Exception ex) {
        logger.error(
            "event=update_display_name topic_arn= " + topicArn + " user_id= " + userId, ex);
      ...
      try {
        ...
        logger.error(
            "event=get_topic_attributes topic_arn= " + topicArn + " user_id= " + userId, ex);
        throw new ServletException(ex);
      }
      ...
      out.println(
          "<form action=\"/webui/cnsuser/editdisplayname?topicArn=" + topicArn + "\" method=POST>");
      ...
      out.println(
          "<p><b>Display Name:</b> <input type='text' name='displayName' size='100' value= '"
              + (attributes.get("DisplayName") == null ? "" : attributes.get("DisplayName"))
              + "'></p>");
