...
public class CNSPublishAction extends CNSAction {
  ...
  public static final String CNS_PUBLISH_QUEUE_NAME_PREFIX =
      CMBProperties.getInstance().getCNSPublishQueueNamePrefix();
  public static final String CNS_ENDPOINT_QUEUE_NAME_PREFIX =
  ...
  /**
   * The method simply gets the information from the user and request to call publish the message
   * passed in the "Message" field in the request parameters, and publish them to all endpoints
   * subscribed to the topic designated by the "TopicArn" field in the request, then take the
  ...
  public boolean doAction(User user, AsyncContext asyncContext) throws Exception {
    ...
    if (awsCredentials == null) {
      awsCredentials =
          new BasicAWSCredentials(
              cnsInternalUser.getAccessKey(), cnsInternalUser.getAccessSecret());
    }
    ...
    if (request.getParameter("MessageStructure") != null) {
      ...
      if (!messageStructure.equals("json")) {
        logger.error(
            "event=cns_publish error_code=InvalidParameters message="
                + message
                + " message_structure="
                + messageStructure
                + " topic_arn="
    ...
    if ((userId == null) || (message == null)) {
      logger.error(
          "event=cns_publish error_code=InvalidParameters message="
              ...
              + userId);
      throw new CMBException(
          CNSErrorCodes.CNS_ValidationError,
    ...
    if (topic == null) {
      logger.error(
          "event=cns_publish error_code=NotFound message="
              + message
              + " topic_arn="
              + topicArn
              + " user_id="
    ...
    String messageAttributeName =
        request.getParameter(CQSConstants.MESSAGE_ATTRIBUTES + ".entry." + index + ".Name");

    while (messageAttributeName != null && !messageAttributeName.equals("")) {
      String messageAttributeValue =
          request.getParameter(
              CQSConstants.MESSAGE_ATTRIBUTES + ".entry." + index + ".Value.StringValue");
      if (messageAttributeValue == null || messageAttributeValue.equals("")) {
        messageAttributeValue =
            request.getParameter(
                CQSConstants.MESSAGE_ATTRIBUTES + ".entry." + index + ".Value.BinaryValue");
      }
      if (messageAttributeValue == null || messageAttributeValue.equals("")) {
        throw new CMBException(
      ...
      String messageAttributeDataType =
          request.getParameter(
              CQSConstants.MESSAGE_ATTRIBUTES + ".entry." + index + ".Value.DataType");
    ...
    } else if (CMBProperties.getInstance().isCNSBypassPublishJobQueueForSmallTopics()
        && topicAttributes != null
        && topicAttributes.getSubscriptionsConfirmed()
            <= CMBProperties.getInstance().getCNSMaxSubscriptionsPerEndpointPublishJob()) {

      // optimization: if there's only one chunk due to few subscribers, write directly into
      // endpoint publish queue bypassing the publish job queue

      logger.debug("event=using_job_queue_overpass");

      List<CNSEndpointPublishJob.CNSEndpointSubscriptionInfo> subscriptions =
          CNSEndpointPublisherJobProducer.getSubscriptionsForTopic(topicArn);

      if (subscriptions != null && subscriptions.size() > 0) {

        List<CNSEndpointPublishJob> epPublishJobs =
            CNSEndpointPublisherJobProducer.createEndpointPublishJobs(cnsMessage, subscriptions);

        if (epPublishJobs.size() != 1) {
          logger.warn(
              "event=unexpected_number_of_endpoint_publish_jobs count=" + epPublishJobs.size());
        }

        for (CNSEndpointPublishJob epPublishJob : epPublishJobs) {
          String handle =
              sendMessageOnRandomShardAndCreateQueueIfAbsent(
                  CNS_ENDPOINT_QUEUE_NAME_PREFIX,
                  CMBProperties.getInstance().getCNSNumEndpointPublishJobQueues(),
                  epPublishJob.serialize(),
    ...
    if (!success) {
      throw new CMBException(
          CMBErrorCodes.InternalError, "Failed to place message on internal cns queue");
    }
  ...
  private String sendMessageOnRandomShardAndCreateQueueIfAbsent(
      String prefix,
      int numShards,
      String message,
      String userId,
    ...
    String relativeQueueUrl =
        com.comcast.cqs.util.Util.getRelativeQueueUrlForName(queueName, userId);
