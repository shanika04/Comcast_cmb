...
public class CNSSubscribeAction extends CNSAction {
  ...
  /**
   * The method simply gets the information from the user and request to call subscribe on the
   * persistence layer, then we take response and generate an XML response and put it in the
   * parameter response
   *
  ...
  public boolean doAction(User user, AsyncContext asyncContext) throws Exception {
    ...
    if ((endpoint == null) || (protocol == null) || (userId == null) || (topicArn == null)) {
      logger.error(
          "event=cns_subscribe error_code=InvalidParameters user_id="
              + userId
              + " topic_arn="
              + topicArn
              + " endpoint="
    ...
    if (!Util.isValidTopicArn(topicArn)) {
      logger.error(
          "event=cns_subscribe error_code=InvalidParameters problem=invalid_arn user_id="
              + userId
              + " topic_arn="
              + topicArn
              + " endpoint="
    ...
    if (protocol.equals("email-json")) {
      ...
      try {
        ...
        logger.error(
            "event=cns_subscribe error_code=InvalidParameters problem=unknown_protocol user_id="
                + userId
                + " topic_arn="
                + topicArn
                + " endpoint="
    ...
    if (!subProtocol.isValidEnpoint(endpoint)) {
      logger.error(
          "event=cns_subscribe error_code=InvalidParameters problem=invalidEndpoint user_id="
              + userId
              + " topic_arn="
              + topicArn
              + " endpoint="
    ...
    CNSSubscription sub =
        PersistenceFactory.getSubscriptionPersistence()
            .subscribe(endpoint, subProtocol, topicArn, userId);
    ...
    if (sub.isConfirmed()) {
      ...
      try {
        ...
        CommunicationUtils.sendMessage(
            topicOwner, subProtocol, endpoint, message, messageId, topicArn, sub.getArn(), false);
      } catch (Exception ex) {
        PersistenceFactory.getSubscriptionPersistence().unsubscribe(sub.getArn());
        if (ex instanceof CMBException) {
          throw new CMBException(
              CNSErrorCodes.CNS_InvalidParameter, "Invalid parameter: " + ex.getMessage());
        } else {
          throw new CMBException(
              CNSErrorCodes.CNS_InvalidParameter,
      ...
      logger.info(
          "event=cns_subscribe_confirmation_request_sent endpoint="
              + endpoint
              + " protocol="
              + protocol
              + " user_id="
