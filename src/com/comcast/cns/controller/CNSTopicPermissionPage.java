  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (params.containsKey("Remove")) {
      ...
      try {
        RemovePermissionRequest removePermissionRequest =
            new RemovePermissionRequest(topicArn, labelSid);
        sns.removePermission(removePermissionRequest);
        logger.debug(
            "event=remove_permission topic_arn="
                + topicArn
                + " label="
                + labelSid
                + " user_id="
    ...
    if (policy != null && !policy.getStatements().isEmpty()) {
      ...
      for (int i = 0; stmts != null && i < stmts.size(); i++) {
        ...
        out.println(
            "<form action=\"/webui/cnsuser/permission/?userId="
                + user.getUserId()
                + "&topicArn="
                + topicArn
                + "\" method=POST>");
    ...
    out.println(
        "<p><a href='' onclick=\"window.open('/webui/cnsuser/addpermission/?topicArn="
            + topicArn
            + "&topicName="
            + Util.getNameFromTopicArn(topicArn)
            + "&userId="
