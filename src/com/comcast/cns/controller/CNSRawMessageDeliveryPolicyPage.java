  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (params.containsKey("Update")) {
      ...
      try {
        ...
        logger.debug(
            "event=set_raw_message_delivery_policy sub_arn=" + subArn + " user_id= " + userId);

      } catch (Exception ex) {
        logger.error(
            "event=set_raw_message_delivery_policy sub_arn=" + subArn + " user_id= " + userId, ex);
      ...
      if (subArn != null) {
        ...
        try {
          ...
          logger.error(
              "event=get_raw_message_delivery_attribute sub_arn=" + subArn + " user_id= " + userId,
              ex);
          throw new ServletException(ex);
        }
      }
      ...
      out.println(
          "<form action=\"/webui/cnsuser/subscription/rawmessagedeliverypolicy?subscriptionArn="
              + subArn
              + "\" method=POST>");
      out.println("<input type='hidden' name='userId' value='" + userId + "'>");
      out.println("<table width='98%'");
      out.println(
          "<tr><td colspan=2><b><font color='orange'>Raw Message Delivery</font></b></td></tr>");
      out.println(
          "<tr><td ><input type='radio' name='rawmessage' value='true' "
              + (rawMessageDelivery ? "checked='true'" : "")
              + ">True</td>");
      out.println(
          "<td ><input type='radio' name='rawmessage' value='false'  "
              + (rawMessageDelivery ? "" : "checked='true'")
              + ">False</td></tr>");
