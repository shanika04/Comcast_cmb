...
public class CNSConfirmSubscriptionAction extends CNSAction {
  ...
  /**
   * The method simply gets the information from the user and request to call confirmSubscription on
   * the persistence layer, then we take response and generate an XML response and put it in the
   * parameter response
   *
  ...
  public boolean doAction(User user, AsyncContext asyncContext) throws Exception {
    ...
    if ((topicArn == null) || (token == null)) {
      logger.error(
          "event=cns_confirmsubscription error_code=InvalidParameters token="
              + token
              + " topic_arn="
              + topicArn
              + " authenticate_on_unsubscribe="
              + authOnUnsubscribeStr);
