...
public class CNSSetSubscriptionAttributesAction extends CNSAction {
  ...
  /**
   * The method simply gets the information from the user and request to set the subscription
   * attributes based on what the user passed as the "AttributeName", and set that attribute to
   * "AttributeValue"
   *
  ...
  public boolean doAction(User user, AsyncContext asyncContext) throws Exception {
    ...
    logger.debug(
        "event=cns_set_subscription_attributes attribute_name="
            + attributeName
            + " attribute_value="
            + attributeValue
            + " subscription_arn="
    ...
    if ((userId == null)
        ...
        || (attributeValue == null)) {
      logger.error(
          "event=cns_set_subscription_attributes error_code=InvalidParameters attribute_name="
              + attributeName
              + " attribute_value="
              + attributeValue
              + " subscription_arn="
    ...
    if (attributeName.equals("DeliveryPolicy")) {
      ...
      PersistenceFactory.getSubscriptionPersistence()
          .setRawMessageDelivery(subscriptionArn, rawMessageDelivery);
    } else {
      logger.error(
          "event=cns_set_subscription_attributes error_code=InvalidParameters attribute_name="
              + attributeName
    ...
    PersistenceFactory.getCNSAttributePersistence()
        .setSubscriptionAttributes(subscriptionAttributes, subscriptionArn);
