...
public class CNSListSubscriptionsByTopicAction extends CNSAction {
  ...
  public boolean doAction(User user, AsyncContext asyncContext) throws Exception {
    ...
    if (request.getParameter("NextToken") != null) {
      ...
      logger.debug(
          "event=cns_list_topics_by_subscription next_token="
              + nextToken
              + " topic_arn="
              + topicArn
              + " user_id="
      ...
      logger.debug(
          "event=cns_list_topics_by_subscription topic_arn=" + topicArn + " userId=" + userId);
    }

    if ((topicArn == null) || (userId == null)) {
      logger.error(
          "event=cns_list_topics_by_subscription error_code=InvalidParameters  topic_arn="
              + topicArn
              + " user_id="
              + userId);
    ...
    if (subscriptions.size() >= 100) {
      ...
      if (nextSubscriptions.size() == 0) {
        ...
        logger.debug(
            "event=cns_subscriptionsbytopic_listed event=next_token_created next_token="
                + nextToken
                + " userid="
                + userId);
      }
    ...
    String out =
        CNSSubscriptionPopulator.getListSubscriptionByTopicResponse(subscriptions, nextToken);
