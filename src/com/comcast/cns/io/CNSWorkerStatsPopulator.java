...
public class CNSWorkerStatsPopulator {
  ...
  public static String getGetWorkerStatsResponse(List<CNSWorkerStats> statsList) {
    ...
    for (CNSWorkerStats stats : statsList) {
      ...
      out +=
          "\t\t\t<RedeliveryQueueSize>"
              + stats.getRedeliveryQueueSize()
              + "</RedeliveryQueueSize>\n";
      out +=
          "\t\t\t<ConsumerOverloaded>" + stats.isConsumerOverloaded() + "</ConsumerOverloaded>\n";
      out +=
          "\t\t\t<CqsServiceAvailable>"
              + stats.isCqsServiceAvailable()
              + "</CqsServiceAvailable>\n";
      out +=
          "\t\t\t<NumPooledHttpConnections>"
              + stats.getNumPooledHttpConnections()
              + "</NumPooledHttpConnections>\n";

      if (stats.getErrorCountForEndpoints() != null
          && stats.getErrorCountForEndpoints().size() > 0) {
