...
public class HTTPEndpointSyncPublisher extends AbstractEndpointPublisher {
  ...
  static {
    ...
    cm.setDefaultMaxPerRoute(
        CMBProperties.getInstance().getCNSPublisherHttpEndpointConnectionsPerRouteSize());
  ...
  public void send() throws Exception {
    ...
    if ((message == null) || (endpoint == null)) {
      logger.debug(
          "event=send_http_request error_code=MissingParameters endpoint="
              + endpoint
              + "\" message=\""
              + message
              + "\"");
    ...
    logger.debug(
        "event=send_sync_http_request endpoint=" + endpoint + "\" message=\"" + msg + "\"");
    ...
    if (statusCode > 200 || statusCode >= 300) {
      ...
      throw new CMBException(
          new CMBErrorCodes(statusCode, "HttpError"),
