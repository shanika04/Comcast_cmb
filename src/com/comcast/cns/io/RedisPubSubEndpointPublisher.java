...
public class RedisPubSubEndpointPublisher extends AbstractEndpointPublisher {
  ...
  protected static int timeoutMS =
      CMBProperties.getInstance().getRedisPubSubEndpointConnectionTimeoutMS();
  ...
  public void send() throws Exception {
    if (jedisPoolMap.containsKey(endpoint)) {
      ...
      if (m.matches()) {
        ...
        JedisPool pool =
            new JedisPool(new JedisPoolConfig(), host, Integer.parseInt(port), timeoutMS, password);
        Jedis connection = pool.getResource();
        long numClients = connection.publish(channel, message.getMessage());
        long timeTaken = System.currentTimeMillis() - startTime;
        logger.debug(
        ...
        logger.error(
            "event=send_redis error=badEndpoint endpoint="
                + endpoint
                + " topicArn="
                + message.getTopicArn());
        return;
