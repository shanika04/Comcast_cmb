...
public class Util {
  ...
  public static String generateCnsTopicSubscriptionArn(
      String topicArn, CnsSubscriptionProtocol protocol, String endpoint)
  ...
  public static String getCnsTopicArn(String subArn) {
    ...
    if (arr.length < 2) {
      throw new IllegalArgumentException(
          "Bad format for subscription. Expected <topic-arn>:UUID Got:" + subArn);
    }
  ...
  private static final Pattern topicPattern =
      Pattern.compile("arn:cmb:cns:[A-Za-z0-9-_]+:[A-Za-z0-9-_]+:[A-Za-z0-9-_]+");
  ...
  private static final Pattern subPattern =
      Pattern.compile("arn:cmb:cns:[A-Za-z0-9-_]+:[A-Za-z0-9-_]+:[A-Za-z0-9-_]+:[A-Za-z0-9-_]+");
  ...
  public static String generateMessageJson(CNSMessage cnsMessage, CnsSubscriptionProtocol prot) {
    ...
    JSONWriter jw = new JSONWriter(writer);
    SimpleDateFormat df =
        new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ"); // Time is in UTC zone. i,e no offset
   ...
   */
  public static int getNextRetryDelay(
      int i,
      int maxBackOffRetries,
      int minDelayTarget,
      int maxDelayTarget,
    ...
    switch (backOffFunction) {
      case linear:
        ...
        // calculate slope given f(maxBackOffRetries) = maxDelayTarget = slope(maxBackOffRetries -
        // 1) + minDelayTarget
        // => slope = (maxDelayTarget - minDelayTarget)/ (maxBackOffRetries - 1)
        double slope =
            (double) (maxDelayTarget - minDelayTarget) / (double) (maxBackOffRetries - 1);
        return (int) (slope * (i - 1) + minDelayTarget);

      case geometric:
        // figure out x using equation: x^(maxBackOffRetries - 1) + minDelayTarget - 1 =
        // maxDelayTarget
      ...
      case arithmetic:
        ...
        // f(i) = a(i-1)^2 + b
        // figure out a using b = minDelayTarget & a(maxBackOffRetries-1)^2 + minDelayTarget =
        // maxDelayTarget
